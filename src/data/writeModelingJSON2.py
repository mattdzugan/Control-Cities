                        ...
                        if cityPop>0:
                            ...
                            if angleBetween<60:
                                ...
                                if ((distToCity<50)|(angleBetween<20)):
                                    ...
                                    cityForNow['stack1D'] = ((cityPop)/(distToCity))
                                    cityForNow['stack2D'] = ((cityPop**0.5)/(distToCity))
                                    cityForNow['stack3D'] = ((cityPop**0.333)/(distToCity))
                                    cityForNow['capac'] = ((cityPop**2)/(distToCity))
                                    cityForNow['middle'] = ((cityPop**3)/(distToCity**2))
                                    citiesForNow.append( cityForNow )                          
                    if len(citiesForNow)>0:
                        citiesForNow = sorted(citiesForNow, key=lambda k: k['stack1D'], reverse=True)
                        predictedCities.append(citiesForNow[0]['id'])                    
                        citiesForNow = sorted(citiesForNow, key=lambda k: k['stack2D'], reverse=True)
                        predictedCities.append(citiesForNow[0]['id'])                  
                        citiesForNow = sorted(citiesForNow, key=lambda k: k['stack3D'], reverse=True)
                        predictedCities.append(citiesForNow[0]['id'])                   
                        citiesForNow = sorted(citiesForNow, key=lambda k: k['capac'], reverse=True)
                        predictedCities.append(citiesForNow[0]['id'])                   
                        citiesForNow = sorted(citiesForNow, key=lambda k: k['middle'], reverse=True)                    
                        predictedCities.append(citiesForNow[0]['id'])
                        ...
                        if len(citiesForNowClose)>0:
                            predictedCities.append(citiesForNowClose[0]['id'])
                        citiesForNowClose = [x for x in citiesForNow if x['dist'] < 80.5] #50 miles
                        citiesForNowClose = sorted(citiesForNowClose, key=lambda k: k['pop'], reverse=True)  
                        if len(citiesForNowClose)>0:
                            predictedCities.append(citiesForNowClose[0]['id'])
            ...
            for direction in ['destWith', 'destAgainst']:
                ...
                if (len(myDestIDs)>0):
                    if (max(myDestIDs)>0):
                        ...
                        citiesForNow = sorted(citiesForNow, key=lambda k: k['stack1D'], reverse=True)
                        if len(citiesForNow)>0:
                            leaders['stack1D'] = citiesForNow[0]['id']
                        else:
                            leaders['stack1D'] = -999
                        
                        citiesForNow = sorted(citiesForNow, key=lambda k: k['stack2D'], reverse=True)
                        if len(citiesForNow)>0:
                            leaders['stack2D'] = citiesForNow[0]['id']
                        else:
                            leaders['stack2D'] = -999
                            
                        citiesForNow = sorted(citiesForNow, key=lambda k: k['stack3D'], reverse=True)
                        if len(citiesForNow)>0:
                            leaders['stack3D'] = citiesForNow[0]['id']
                        else:
                            leaders['stack3D'] = -999
                        ...
                        citiesForNow = sorted(citiesForNow, key=lambda k: k['middle'], reverse=True)
                        if len(citiesForNow)>0:
                            leaders['middle'] = citiesForNow[0]['id']
                        else:
                            leaders['middle'] = -999
                        ...
                        if len(citiesForNow)>0:
                            ...
                            citiesForNowClose = sorted(citiesForNowClose, key=lambda k: k['pop'], reverse=True)  
                            if len(citiesForNowClose)>0:
                                leaders['max50'] = citiesForNowClose[0]['id']
                            else:
                                leaders['max50'] = -999
                        else:
                            leaders['max5H'] = -999
                        ...
                        isTrue = {}
                        isTrue['stack1D'] = (str(leaders['stack1D']) in myDestIDs)
                        isTrue['stack2D'] = (str(leaders['stack2D']) in myDestIDs)
                        isTrue['stack3D'] = (str(leaders['stack3D']) in myDestIDs)
                        isTrue['capac'] = (str(leaders['capac']) in myDestIDs)
                        isTrue['middle'] = (str(leaders['middle']) in myDestIDs)
                        isTrue['max5H'] = (str(leaders['max5H']) in myDestIDs)
                        isTrue['max1H'] = (str(leaders['max1H']) in myDestIDs)
                        isTrue['max50'] = (str(leaders['max50']) in myDestIDs)
...
counts = {'Count':0,            'stack1D':0,  'stack2D':0,  'stack3D':0,  'capac':0,  'middle':0,  'max5H':0,  'max1H':0,  'max50':0}
hists  = {'names':[],'dirs':[], 'stack1D':[], 'stack2D':[], 'stack3D':[], 'capac':[], 'middle':[], 'max5H':[], 'max1H':[], 'max50':[]}                
    ...
    for direction in ['destWith', 'destAgainst']:
        if (myOrientation=='South'):
            ...
            if (direction=='destWith'):
                ...
                myDir = 'Westbound'
        N_thisDir = 0
        counts_thisDir = {'stack1D':0, 'stack2D':0, 'stack3D':0, 'capac':0, 'middle':0, 'max5H':0, 'max1H':0, 'max50':0}
        for ii in range(N):
            seg = path[ii]
            truths = seg['truths']
            if direction in truths.keys():
                m = truths[direction]
                N_thisDir += 1
                counts_thisDir['stack1D'] += int(m['stack1D'])
                counts_thisDir['stack2D'] += int(m['stack2D'])
                counts_thisDir['stack3D'] += int(m['stack3D'])
                counts_thisDir['capac']   += int(m['capac'])
                counts_thisDir['middle']  += int(m['middle'])
                counts_thisDir['max5H']   += int(m['max5H'])
                counts_thisDir['max1H']   += int(m['max1H'])
                counts_thisDir['max50']   += int(m['max50'])
        # if it's big enough, add it to hist
        if (N_thisDir>20):
            hists['names'].append(myName)
            hists['dirs'].append(myDir)
            hists['stack1D'].append(counts_thisDir['stack1D']/float(N_thisDir))
            hists['stack2D'].append(counts_thisDir['stack2D']/float(N_thisDir))
            hists['stack3D'].append(counts_thisDir['stack3D']/float(N_thisDir))
            hists['capac'].append(counts_thisDir['capac']/float(N_thisDir))
            hists['middle'].append(counts_thisDir['middle']/float(N_thisDir))
            hists['max5H'].append(counts_thisDir['max5H']/float(N_thisDir))
            hists['max1H'].append(counts_thisDir['max1H']/float(N_thisDir))
            hists['max50'].append(counts_thisDir['max50']/float(N_thisDir))
        # add everything to the master count list
        counts['Count'] += N_thisDir
        counts['stack1D'] += counts_thisDir['stack1D']
        counts['stack2D'] += counts_thisDir['stack2D']
        counts['stack3D'] += counts_thisDir['stack3D']
        counts['capac'] += counts_thisDir['capac']
        counts['middle'] += counts_thisDir['middle']
        counts['max5H'] += counts_thisDir['max5H']
        counts['max1H'] += counts_thisDir['max1H']
        counts['max50'] += counts_thisDir['max50']
                     
overview = []
histData = []
for method in ['stack1D','stack2D','stack3D','capac','middle','max5H','max1H','max50']:
